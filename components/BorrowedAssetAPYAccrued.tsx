...
import { BorrowedAssetDataItem, useAaveData } from "../hooks/useAaveData";
...
export const BorrowedAssetAPYAccrued = ({
  ...
  const accrual = useAccruedInterest(
    currentMarket,
    resolvedAddress,
    asset?.asset?.variableDebtTokenAddress,
    "borrow"
  );
  ...
  const valueDisplay: string = `${formatTokenAmount(
    accruedValue,
    i18n.locale
  )} ${asset.asset.symbol} `;
  const dateDisplay: string = accrual.sinceTimestamp
    ? ` since ${i18n.date(new Date(accrual.sinceTimestamp * 1000), {
        dateStyle: "medium",
      })}`
    : "";
