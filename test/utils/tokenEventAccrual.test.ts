...
let eventOrder = 0;
const event = (partial: Partial<TokenFlowEvent> & Pick<TokenFlowEvent, "kind" | "value">): TokenFlowEvent => ({
    blockNumber: (eventOrder += 1),
    logIndex: 0,
    ...partial,
});
...
describe("getPrincipalFlow", () => {
    it("separates principal from credited interest on Mint", () => {
        ...
        const flow = getPrincipalFlow(event({ kind: "Mint", value: usdc(102), balanceIncrease: usdc(2) }));
    ...
    it("handles the burn-side Mint (withdrawal smaller than accrued interest)", () => {
        ...
        const flow = getPrincipalFlow(event({ kind: "Mint", value: usdc(3), balanceIncrease: usdc(8) }));
    ...
    it("adds credited interest back to the principal on Burn", () => {
        ...
        const flow = getPrincipalFlow(event({ kind: "Burn", value: usdc(98), balanceIncrease: usdc(2) }));
        expect(flow.toString()).toBe(usdc(-100));
    });

    it("treats transfers at face value", () => {
        expect(getPrincipalFlow(event({ kind: "TransferIn", value: usdc(50) })).toString()).toBe(usdc(50));
        expect(getPrincipalFlow(event({ kind: "TransferOut", value: usdc(50) })).toString()).toBe(usdc(-50));
    });
});
...
describe("getAccruedInterest", () => {
    ...
    it("handles a position acquired only via transfer", () => {
        const events = [
            event({ kind: "TransferIn", value: usdc(100) }),
        ];
...
describe("findFirstPrincipalEvent", () => {
    it("finds the earliest principal-adding event by chain order", () => {
        const later = event({ kind: "Mint", value: usdc(50), balanceIncrease: usdc(0) });
        const earlier = event({ kind: "Mint", value: usdc(100), balanceIncrease: usdc(0), blockNumber: 1 });
        const found = findFirstPrincipalEvent([later, earlier]);
        expect(found).toBe(earlier);
    });

    it("skips burn-side Mints (they remove principal)", () => {
        const burnSideMint = event({ kind: "Mint", value: usdc(3), balanceIncrease: usdc(8) });
    ...
    it("returns undefined when nothing added principal", () => {
        ...
        expect(findFirstPrincipalEvent([event({ kind: "TransferOut", value: usdc(1) })])).toBeUndefined();
    });
});
...
describe("clampRoundingDust", () => {
    ...
    it("leaves materially negative values alone", () => {
        ...
        expect(clampRoundingDust(BigNumber.from(-1_000_000), 61).toString()).toBe("-1000000");
    });

    it("never touches zero or positive values", () => {
        expect(clampRoundingDust(BigNumber.from(0), 61).toString()).toBe("0");
        expect(clampRoundingDust(BigNumber.from(12345), 0).toString()).toBe("12345");
    });
});
