...
export function useAccruedInterest(
    ...
    const isReady: boolean = !!marketId?.length && !!user?.length && !!tokenAddress?.length;
    const key = `${marketId}:${tokenAddress}:${user}:${side}`.toLowerCase();
    const [state, setState] = useState<AccruedInterestState>(cache.get(key) ?? PENDING);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                next = { isFetching: false, fetchError: err?.message ?? "Failed to fetch" };
            }
            cache.set(key, next);
