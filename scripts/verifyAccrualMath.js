...
(async () => {
  ...
  for (const { log, args } of candidates) {
    ...
    const indexMathAccrued = args.value
      .mul(indexNow.sub(args.index))
      .div(args.index);
  ...
  if (verified === 0)
    console.log("No clean single-event positions found in window.");
})().catch((e) => {
  console.error("ERR:", (e.message || String(e)).slice(0, 300));
  process.exit(1);
});
